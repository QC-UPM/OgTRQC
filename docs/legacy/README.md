# Historical quantum-geometric prototype (superseded)

This archived README describes the pre-paper prototype. Its claims about recoverability-driven forces, quantum memory, curvature, and storage capacity are **not conclusions of the proton-transport paper**. Use the repository root README and the `proton` API for the current implementation. Historical command examples may require `python -m octa_gtrqc_sim.main`.
