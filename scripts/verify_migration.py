"""Compare a native campaign with freshly replayed reference notebooks.

Run with Poetry after both complete campaigns. The comparison fails closed and
...
def verify(native: Path, reference: Path) -> dict:
    ...
    Args:
        ...
        reference: Root produced by ``proton-reference``.
    ...
    main_root = reference / 'Hx_NdNiO3_single_cell_evidence_calibrated_PoC_v3_1'
    main = json.loads((main_root / 'machine_readable/summary.json').read_text())
    statuses = list(reference.glob('*/execution.json'))
    require('all_four_notebooks_executed', len(statuses) == 4 and all(json.loads(p.read_text())['success'] for p in statuses))
    require('original_80_gate_audit', main['audit']['passed'] == main['audit']['total'] == 80)
    ...
    r2 = reference / 'proton_R2_5_identifiability/proton_R2_5_outputs'
    ...
    r3 = reference / 'proton_R3_full_generator_audit/proton_R3_outputs/new_results'
    ...
    addendum = reference / 'proton_review_addendum/proton_review_addendum_results'
    ...
    record = dict(all_passed=all(c['passed'] for c in checks), checks=checks,
                  scope='Numerical migration parity, not new physical validation. Original notebook terminology is retained only in source artifacts.')
    return record
...
def main() -> None:
    ...
    parser.add_argument('--native', type=Path, default=Path('paper_results'))
    parser.add_argument('--reference', type=Path, default=Path('reference_results'))
    parser.add_argument('--output', type=Path, default=Path('validation/migration.json'))
